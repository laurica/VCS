...
Diff DiffBuilder::build() {
  ...
  // now we need to properly set the 'new line number' field of the diff
  // elements
  // this only applies to insertions (deleted lines don't have anything
  // corresponding in the new file)
  ...
  while (it != insertions.end()) {
    ...
    while (delIt != deletions.end() &&
	   delIt->getBaseStartingLine() < it->getBaseStartingLine()) {
